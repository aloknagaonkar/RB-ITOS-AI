...
def replay_immediate_exit(bs: list[Bar], sd: date):
    ...
    for i, b in enumerate(bs):
        ...
        if None in (b.rsi, b.ema, b.wma):
            rows.append({
                ...
                "rsi_gt_ema": "", "rsi_gt_wma": "", "ema_gt_wma": "",
                "rsi_slope_up": "", "ema_slope_up": "",
                "rsi_cross_ema_up": "", "rsi_cross_wma_down": "",
                "full_alignment": "", "armed": yn(armed), "active": yn(active),
                "source": source, "entry_time": entry_time, "entry_close": f2(entry_close),
                "points_from_entry_close": "", "event": "INDICATOR_WARMUP",
            })
            continue
        ...
        # IMMEDIATE structural exit: first RSI cross below WMA21.
        ...
        # Opening path. Only if no active regime after any exit above.
        if t == "09:15" and full_alignment(b):
            ...
            if b.rsi > b.wma and not active:
                ...
                events.append((t, "ENTRY_OPENING_BULLISH_CONFIRMED", source, entry_time, entry_close, None, None))
            else:
                event = event or "OPENING_REJECTED_0925"
            opening_candidate = False
            opening_holding = False

        # Path 1: RSI crosses EMA3 upward -> armed.
        if not active and rsi_cross_ema:
            armed = True
            armed_time = t
            event = event or "PATH1_ARMED_RSI_CROSS_EMA3_UP"

        # Confirmation can happen on same candle as arming.
        if (
            not active and armed
            and b.rsi > b.wma
            and b.ema > b.wma
            ...
            entry_time = t
            entry_close = b.close
            event = event or "ENTRY_PATH1_BULLISH_CONFIRMED"
            events.append((t, "ENTRY_PATH1_BULLISH_CONFIRMED", source, entry_time, entry_close, None, None))
            armed = False
            armed_time = ""
        ...
        rows.append({
            ...
            "open": f2(b.open), "high": f2(b.high), "low": f2(b.low), "close": f2(b.close),
            "rsi9": f2(b.rsi), "ema3_rsi": f2(b.ema), "wma21_rsi": f2(b.wma),
            "rsi_gt_ema": yn(b.rsi > b.ema), "rsi_gt_wma": yn(b.rsi > b.wma),
            "ema_gt_wma": yn(b.ema > b.wma),
    ...
    if active:
        events.append((
            bs[-1].ts.strftime("%H:%M"), "SESSION_CENSORED_ACTIVE",
...
def main():
    ...
    ap.add_argument("--dates", nargs="+", default=["2026-09-18", "2026-09-21", "2026-09-22"])
    ap.add_argument("--warmup-calendar-days", type=int, default=45)
    ap.add_argument("--token-env", default="UPSTOX_ACCESS_TOKEN")
    ap.add_argument("--output-dir", default="data/historical-evidence/hilega-milega-immediate-exit-chart-check")
    args = ap.parse_args()
    ...
    for sd in target_dates:
        ...
        print(
            ...
            f"{'R>E':>4} {'R>W':>4} {'E>W':>4} {'RUP':>4} {'EUP':>4} {'ACTIVE':>6} EVENT"
        )
        print("-" * 108)
        for r in rows:
            print(
                f"{r['time']:5} {r['close']:>9} {r['rsi9']:>8} {r['ema3_rsi']:>8} {r['wma21_rsi']:>8} "
                f"{r['rsi_gt_ema']:>4} {r['rsi_gt_wma']:>4} {r['ema_gt_wma']:>4} "
