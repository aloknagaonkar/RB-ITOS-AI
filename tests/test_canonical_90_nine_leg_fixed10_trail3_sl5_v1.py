from market_lab.canonical_90_nine_leg_fixed10_trail3_sl5_v1 import (
...
def test_initial_stop_is_minus_five_percent():
    inst = "X"
    bars = {
        inst: {
            "2026-08-25T09:37:00+05:30": make_bar(
                "2026-08-25T09:37:00+05:30", 100, 102, 94, 96
            )
        }
    }
    ...
    assert abs(r["exit_price"] - 95.0) < 1e-9
    assert abs(r["net_return_pct"] - (-5.5)) < 1e-9
...
def test_time_exit_if_neither_stop_nor_trail_exit():
    ...
    for i in range(1, 16):
        ...
        bars[inst][ts] = make_bar(ts, 100, 105, 96, 103 if i == 15 else 100)
