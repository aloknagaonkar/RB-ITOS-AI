from pathlib import Path

import market_lab.hilega_directional_historical_ui_v1 as m


def _write(path: Path, text: str):
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(text, encoding="utf-8")


def test_directional_historical_dashboard_combines_ce_and_pe(tmp_path, monkeypatch):
    day = "2026-09-17"
    droot = tmp_path/"directional"
    ceroot = tmp_path/"ce"
    peroot = tmp_path/"pe"

    _write(
        droot/day/"directional-trades.csv",
        "session_date,direction,entry_time,entry_event,entry_price,exit_time,exit_event,exit_price,points,outcome,holding_minutes\n"
        f"{day},BULLISH,09:25,ENTRY_BULL,100,10:20,EXIT_BULL,110,10,POSITIVE,55\n"
        f"{day},BEARISH,10:30,ENTRY_BEAR,110,11:30,EXIT_BEAR,100,10,POSITIVE,60\n",
    )

    for root, side, t, event in [
        (ceroot, "CE", "09:25", "ENTRY_BULL"),
        (peroot, "PE", "10:30", "ENTRY_BEAR"),
    ]:
        prefix = side.lower()
        _write(
            root/day/f"{prefix}-shadow-trades.csv",
            "session_date,trade_index,entry_time,exit_time,entry_event,exit_event,signal_spot,atm,expiry,status,complete_legs,issue\n"
            f"{day},1,{t},10:00,{event},EXIT,23250,23250,2026-09-22,CLOSED,5,\n",
        )
        _write(
            root/day/f"{prefix}-shadow-legs.csv",
            "trade_index,relation_to_atm,strike,side,instrument_key,entry_timestamp,entry_open,exit_timestamp,exit_open,realized_points,realized_return_pct,mfe_points,mae_points\n"
            + "\n".join(
                f"1,{rel},{23250+rel*50},{side},{side}{rel},{day}T09:30:00+05:30,100,{day}T10:05:00+05:30,110,10,10,20,-2"
                for rel in (-2,-1,0,1,2)
            ) + "\n",
        )

    monkeypatch.setattr(m, "DIRECTIONAL_ROOT", droot)
    monkeypatch.setattr(m, "CE_ROOT", ceroot)
    monkeypatch.setattr(m, "PE_ROOT", peroot)

    result = m.build_directional_historical_dashboard(day)
    assert result["trade_count"] == 2
    assert result["by_direction"]["BULLISH"]["projected_trade_count"] == 1
    assert result["by_direction"]["BEARISH"]["projected_trade_count"] == 1
    assert result["trades"][0]["direction"] == "BULLISH"
    assert result["trades"][0]["option_side"] == "CE"
    assert len(result["trades"][0]["legs"]) == 5
    assert result["trades"][1]["direction"] == "BEARISH"
    assert result["trades"][1]["option_side"] == "PE"
