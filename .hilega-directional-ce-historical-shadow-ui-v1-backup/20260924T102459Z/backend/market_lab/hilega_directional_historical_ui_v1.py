...
def _bullish_trade_entry_hhmm(trade: dict[str, Any]) -> str | None:
    # Directional replay entry_time is the strategy signal-bar time.
    # Option acquisition happens at the following boundary, so matching
    # signal_boundary/leg entry first would incorrectly reject valid trades.
    v = _hhmm(trade.get("signal_bar"))
    if v:
        return v

    # Compatibility only for older projected evidence that may not carry
    # signal_bar. Never alter or synthesize a price.
    for key in ("signal_boundary", "entry_timestamp"):
