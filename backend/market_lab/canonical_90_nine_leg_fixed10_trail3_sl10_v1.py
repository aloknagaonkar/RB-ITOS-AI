...
MODEL = "CANONICAL_90_NINE_LEG_FIXED10_TRAIL3_SL10_V1"
POLICY_ID = "SL10_TRAIL3_AFTER_PLUS10PTS_TIME15"
...
def _summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    vals = [float(r["net_return_pct"]) for r in rows if r.get("available") is True]
...
def replay_fixed_points_policy(
    *,
    ...
    for minute in range(1, MAX_HOLD_MINUTES + 1):
        ...
        # Existing stop is active at bar open and throughout the current bar.
        if o <= active_stop:
            exit_timestamp = ts
            exit_price = o
        ...
        # Only after current-bar stop evaluation do we observe the bar's high.
        # Any new trail becomes active NEXT BAR ONLY.
        if h > best_price:
            best_price = h
        ...
        if trail_armed:
            next_stop = max(
                initial_stop,
                best_price - TRAIL_DISTANCE_POINTS,
            )
    ...
    *,
    ...
    result = {
        ...
        "common_complete_signal_comparison": {
            ...
            "note": (
                "Comparison uses only signals where all nine legs replay successfully."
            ),
        },
        "trades": trades,
        "governance": {
