...
MODEL = "CANONICAL_90_FOUR_LEG_OPTION_REPLAY_V1"
...
def load_coverage(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(doc.get("legs"), list):
        raise ValueError("four-leg coverage artifact missing top-level legs list")
    return doc
...
def _common_complete_event_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    complete_ids = {
        ...
        if len(group) == 4
        and {r["leg"] for r in group} == {"ATM", "OTM1", "OTM2", "OTM3"}
...
def write_csv(rows: list[dict[str, Any]], path: str | Path) -> None:
    ...
    fields = [
        ...
        "leg","strike_offset","c1_timestamp","c2_timestamp","futures_state",
        "option_side","moving_atm","strike","instrument_key",
...
def run(
    *,
    ...
    result = {
        "status": "PASS",
        "model": MODEL,
        "role": "DESCRIPTIVE_FOUR_LEG_REPLAY_ON_EXPOSED_CANONICAL_90_POPULATION",
        ...
        "complete_four_leg_signal_count": (
            len({str(r["event_id"]) for r in common})
            if common else 0
        ),
        "policy_id": POLICY_ID,
        ...
        "summary_all_legs": overall,
        "by_leg": by_leg,
        "by_direction_all_legs": by_direction,
        "by_block_all_legs": by_block,
        "common_complete_signal_comparison": {
            ...
            "by_leg": common_by_leg,
            "note": (
                "Leg comparison uses only signals for which all four legs are "
                "available, preventing unequal-denominator bias."
            ),
        },
        "excluded_session_end_censored_legs": [
            {
                "event_id": r["event_id"],
...
def main() -> None:
    ...
    print(json.dumps({
        ...
        "complete_four_leg_signal_count": r["complete_four_leg_signal_count"],
