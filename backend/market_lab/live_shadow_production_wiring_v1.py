...
class LiveShadowProductionCoordinatorV1:
    def __init__(self,*,engine,config_id,market_sources,events_path,health_path):
    ...
    def process_checkpoint(self,checkpoint):
        checkpoint=floor_5m(checkpoint);self.bootstrap(checkpoint);snap=select_first_snapshot_at_or_after(self.engine,self.config_id,checkpoint)
        if snap is None:self.health.append({"model":MODEL,"checkpoint":checkpoint.isoformat(),"state":"MISSING","reason":"NO_SNAPSHOT_WITHIN_30S"});return {"status":"MISSING_SNAPSHOT"}
        self.features.add_snapshot(snap,checkpoint_timestamp=checkpoint);cp=self.features.build_current();self.health.append({"model":MODEL,"checkpoint":checkpoint.isoformat(),"source_received_at":cp.source_received_at.isoformat(),"source_delay_ms":cp.source_delay_ms,"health_state":cp.health_state,"health_allowed":cp.health_allowed,"health_reason":cp.health_reason,"all3_state":cp.all3_state})
        for oid,s in list(self.adapter.states().items()):
            if s.status!="DETECTED" or not s.all3_candle1_timestamp:continue
            c1=datetime.fromisoformat(s.all3_candle1_timestamp)
            if c1+timedelta(minutes=5)!=checkpoint:continue
            try:futures=self.sources.futures_oi_at_checkpoint(checkpoint);fstate=futures.state if futures.health_allowed else None
            except Exception:self.adapter.shadow.incomplete(oid,checkpoint,"FUTURES_DATA_UNAVAILABLE");continue
            outcome=self.adapter.on_candle2(oid,to_shadow_checkpoint(cp,futures_oi_state=fstate))
            if outcome=="CLASSIFIED":
                instrument=cp.ce_instrument_key if s.direction=="BULLISH" else cp.pe_instrument_key
                if not instrument:self.adapter.shadow.incomplete(oid,checkpoint,"NO_EXACT_ATM_INSTRUMENT")
                else:self.adapter.on_exact_option_resolved(oid,ExactOptionResolution(checkpoint,cp.moving_atm,instrument))
        oid=self.adapter.on_new_all3(to_shadow_checkpoint(cp,futures_oi_state=None),cp.previous_directional_all3)
    ...
    def process_entries_and_open_trades(self,now):
        ...
        for oid,s in list(self.adapter.states().items()):
            if s.status=="OPTION_RESOLVED" and s.confirmation_timestamp and s.option_instrument_key:
                ...
                if len(match)==1:self.adapter.on_exact_next_minute_open(oid,ExactNextMinuteOpen(entry_ts,match[0].open))
                elif now>=entry_ts+timedelta(minutes=2):self.adapter.shadow.incomplete(oid,now,"MISSING_EXACT_NEXT_MINUTE_OPEN")
        ...
        for oid,s in list(self.adapter.states().items()):
            ...
            for bar in completed:
                ...
                if not h.allowed:self.adapter.shadow.incomplete(oid,bar.timestamp,h.reason or "OPTION_MINUTE_UNHEALTHY");break
                status=self.adapter.on_option_minute(oid,to_shadow_option_minute(bar,h))
                if status in FINAL:break
            results.append((oid,self.adapter.states()[oid].status))
        return results
