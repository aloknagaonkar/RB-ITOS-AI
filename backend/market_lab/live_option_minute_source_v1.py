...
class CompletedOptionMinute:
    instrument_key:str; timestamp:datetime; open:float; high:float; low:float; close:float; volume:float|None=None
@dataclass(frozen=True)
class OptionMinuteHealth:
    state:str; allowed:bool; reason:str|None
...
def validate_option_minute(bar,*,expected_instrument_key,previous_timestamp):
    ...
    if previous_timestamp is not None:
        if bar.timestamp<=previous_timestamp:return OptionMinuteHealth("OUT_OF_ORDER",False,"OPTION_OUT_OF_ORDER")
        if (bar.timestamp-previous_timestamp).total_seconds()!=60:return OptionMinuteHealth("MISSING",False,"OPTION_1M_GAP")
    return OptionMinuteHealth("HEALTHY",True,None)
class LiveOptionMinuteSourceV1:
    def __init__(self,instrument_key:str,*,previous_timestamp:datetime|None=None):
        if not instrument_key:raise ValueError("instrument_key required")
        self.instrument_key=instrument_key;self.previous_timestamp=previous_timestamp
    def process(self,bar):
        r=validate_option_minute(bar,expected_instrument_key=self.instrument_key,previous_timestamp=self.previous_timestamp)
        if r.allowed:self.previous_timestamp=bar.timestamp
        return r
