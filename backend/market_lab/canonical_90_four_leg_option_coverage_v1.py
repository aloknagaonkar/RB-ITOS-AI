...
MODEL = "CANONICAL_90_FOUR_LEG_OPTION_COVERAGE_V1"
...
def run(
    *,
    ...
    by_leg = {
        ...
        for label in ("ATM","OTM1","OTM2","OTM3")
    }
    by_direction = {
        d: dict(Counter(
            x["coverage_status"] for x in legs if x["direction"] == d
        ))
    ...
    complete = sum(
        len(v) == 4 and all(x["coverage_status"] == "READY" for x in v)
    ...
    result = {
        ...
        "expected_leg_count": len(events) * 4,
        ...
        "complete_four_leg_signal_count": complete,
...
def main() -> None:
    ...
    print(json.dumps({
        ...
        "complete_four_leg_signal_count": result["complete_four_leg_signal_count"],
