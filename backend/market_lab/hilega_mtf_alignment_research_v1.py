...
def _parse_dt(value: str, session_date: date | None = None) -> datetime:
    value = str(value).strip()

    # Directional replay trade CSV stores entry/exit times as HH:MM.
    if len(value) == 5 and value[2] == ":":
        if session_date is None:
            raise ValueError(f"SESSION_DATE_REQUIRED_FOR_TIME_ONLY:{value}")
    ...
    *,
    ...
    for d in dates:
        ...
        for trade_index, trade in enumerate(trades, start=1):
            ...
            baseline_entry_ts = _parse_dt(trade["entry_time"], d)
            exit_ts = _parse_dt(trade["exit_time"], d)
