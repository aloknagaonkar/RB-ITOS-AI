...
def recover_current_day_directional_timeline(
    *,
    ...
    # Hilega directional recovery only needs exact current-day evidence
    # through the 14:55 strategy candle. Do not let unrelated trailing
    # market-data gaps after the strategy cutoff invalidate recovery.
    supplemental_1m_for_strategy = [
        candle
        for candle in supplemental_1m
        if candle.timestamp.astimezone(IST).strftime("%H:%M") <= "14:59"
    ]

    supplemental_5m = (
        aggregate_exact_5m(supplemental_1m_for_strategy, session_date)
        if supplemental_1m_for_strategy
        else []
    )
    ...
    manifest = {
        ...
        "supplemental_strategy_window_1m_candles": len(
            supplemental_1m_for_strategy
        ),
        "strategy_recovery_through": "14:55",
        **source_counts,
