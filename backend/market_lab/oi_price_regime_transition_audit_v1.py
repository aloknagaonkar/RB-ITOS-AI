...
MODEL = "OI_PRICE_REGIME_TRANSITION_AUDIT_V1"
IST = "+05:30"
...
def build_audit(date: str, positioning_file: Path, futures_csv: Path, option_ohlc_file: Path | None = None, strike_step: float = 50.0, wings: int = 2, labels: list[LabelRange] | None = None):
    labels = labels or []
    ...
    rows=[]; prev_pcr=None; prev_fc=None; prev_foi=None; bull_streak=0; bear_streak=0
    for ts in cps:
        ...
        pts=(dt(ts)-timedelta(minutes=5)).isoformat()
        pce,_,_=sum_side(pts,moving,"CE",exact,fallback); ppe,_,_=sum_side(pts,moving,"PE",exact,fallback)
        ce_d=(ce-pce) if ce is not None and pce is not None else None; pe_d=(pe-ppe) if pe is not None and ppe is not None else None
        imb=(pe_d-ce_d) if ce_d is not None and pe_d is not None else None
        pcr=(pe/ce) if ce not in (None,0) and pe is not None else None; pcr_d=(pcr-prev_pcr) if pcr is not None and prev_pcr is not None else None
        ...
        pc=(fc-prev_fc) if fc is not None and prev_fc is not None else None; oc=(foi-prev_foi) if foi is not None and prev_foi is not None else None
        ...
        rows.append({
            ...
            "pcr_current":pcr,"pcr_change_5m":pcr_d,"pcr_change_sign":sign(pcr_d),
            ...
            "futures_bar_start":fr["bar_start"] if fr else None,"futures_close":fc,"futures_price_change_5m":pc,"futures_oi":foi,"futures_oi_change_5m":oc,
        ...
        if pcr is not None: prev_pcr=pcr
        if fc is not None: prev_fc=fc
        if foi is not None: prev_foi=foi
    return {"status":"PASS","model":MODEL,"session_date":date,"positioning_file":str(positioning_file),"option_ohlc_file":str(option_ohlc_file) if option_ohlc_file else None,"futures_csv":str(futures_csv),"morning_fixed_atm":morning_atm,"fixed_strikes":fixed_strikes,"rows":rows}
...
def main():
    ...
    ap.add_argument("--strike-step",type=float,default=50.0); ap.add_argument("--moving-wings",type=int,default=2); ap.add_argument("--label",action="append",default=[])
    ...
    report=build_audit(a.session_date,pos,fut,opt,a.strike_step,a.moving_wings,[parse_label(x) for x in a.label])
