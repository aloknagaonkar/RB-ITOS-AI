...
# This deployment intentionally uses a weekday-only session calendar. The
# runtime still blocks Saturday and Sunday, while Monday-Friday exchange
# holidays are treated as ordinary weekdays unless the operator supplies them.
$env:MARKET_TREND_RESEARCH_CALENDAR_VERIFIED = "true"
if ($null -eq $env:MARKET_TREND_RESEARCH_HOLIDAYS) {
    $env:MARKET_TREND_RESEARCH_HOLIDAYS = ""
}
