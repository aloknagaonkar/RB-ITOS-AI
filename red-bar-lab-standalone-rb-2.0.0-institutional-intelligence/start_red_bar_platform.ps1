...
function Stop-OrphanedRedBarProcesses {
    ...
    foreach ($process in $orphans) {
        ...
        try {
            ...
            Write-Host ("Stopped remaining Red Bar CLI/runtime process PID {0}." -f $process.ProcessId) -ForegroundColor Green
        }
        catch {
            Write-Host ("Unable to stop remaining Red Bar process PID {0}: {1}" -f $process.ProcessId, $_.Exception.Message) -ForegroundColor Yellow
        }
    }
}
