...
def verified_calendar() -> StaticExchangeSessionCalendar:
    verified = os.getenv("MARKET_TREND_RESEARCH_CALENDAR_VERIFIED", "false").strip().lower() in {
        "1", "true", "yes", "on"
    }
...
def main() -> int:
    ...
    print(
        ...
        f"lifecycle={snapshot.lifecycle_state.value} "
    ...
    return 0 if snapshot.quality.state.value in {
        "READY",
        "MORNING_REFERENCE_UNAVAILABLE",
        "MORNING_OI_BASELINE_UNAVAILABLE",
    } else 2
