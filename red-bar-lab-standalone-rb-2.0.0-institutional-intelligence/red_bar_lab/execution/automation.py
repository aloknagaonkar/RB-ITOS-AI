...
class RedBarPaperAutomationService:
    ...
    def __init__(
        ...
        *,
        ...
        directional_regime_aligned_bonus: float = 5.0,
        hold_on_directional_regime_conflict: bool = True,
    ):
        ...
        self.directional_regime_aligned_bonus = float(
            directional_regime_aligned_bonus
        )
        self.hold_on_directional_regime_conflict = bool(
            hold_on_directional_regime_conflict
        )
    ...
    def process_new_signals(
        ...
        *,
        ...
        for signal in signals:
            ...
            directional_policy = evaluate_directional_regime_policy(
                "UNAVAILABLE",
                aligned_bonus=self.directional_regime_aligned_bonus,
            )
            if self.directional_regime_reference is not None:
                try:
                    ...
                    directional_policy = (
                        evaluate_directional_regime_policy(
                            directional_reference.status,
                            aligned_bonus=(
                                self.directional_regime_aligned_bonus
                            ),
                        )
                    )
                    self._record_state(
                        ...
                        detail=(
                            ...
                            f"policy_action={directional_policy.action}; "
                            f"candidate_bonus="
                            f"{directional_policy.candidate_score_bonus:.1f}; "
                            "mode=ACTIVE_CONFIRMATION_FILTER"
                        ),
                        score=(
            ...
            if (
                ...
                self._record_state(
                    signal_id=signal_id,
                    state="DIRECTIONAL_REGIME_HOLD",
                    detail=(
                        f"status={directional_policy.status}; "
                        f"action={directional_policy.action}; "
                        f"reason={reason}; "
                        "new_candidate_creation=BLOCKED; "
                        "paper_order_creation=BLOCKED"
                    ),
                    score=(
                        directional_reference.alignment_score
                        if directional_reference is not None
                        else 0.0
                    ),
                )
                skipped += 1
            ...
            try:
                ...
                if directional_policy.candidate_score_bonus > 0:
                    scores = [
                        replace(
                            candidate,
                            total_score=(
                                directional_policy.adjusted_score(
                                    candidate.total_score
                                )
                            ),
                            reason=(
                                f"{candidate.reason}; "
                                f"DIRECTIONAL_REGIME_BONUS="
                                f"{directional_policy.candidate_score_bonus:.1f}"
                            ),
                        )
                    ...
                    self._record_state(
                        signal_id=signal_id,
                        state="DIRECTIONAL_REGIME_BONUS_APPLIED",
                        detail=(
                            f"status={directional_policy.status}; "
                            f"bonus={directional_policy.candidate_score_bonus:.1f}; "
                            "score_cap=100"
                        ),
                ...
                if str(
                    ...
                    candidate_limit = max(
                        1,
                        int(signal.get("candidate_limit") or 1),
                    )
                    scores = scores[:candidate_limit]
                    self._record_state(
                        signal_id=signal_id,
                        state=(
                            "EARLY_1M_CANDIDATE_LIMIT_APPLIED"
                        ),
                        detail=(
                            f"candidate_limit={candidate_limit}; "
                            "reason=EARLY_ENTRY_RISK_CONTROL"
                        ),
                        score=(
                ...
                opposite_red_bar = resolve_opposite_red_bar(
                    signal=signal,
                    signals=signals,
                )
                entry_mode = (
