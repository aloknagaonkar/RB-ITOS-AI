...
def _has_observed_option_data(latest: Mapping[str, object]) -> bool:
    return any(
    ...
    *,
    ...
    market_hours_status = _diagnostic_status(
        ...
        default=(
            ...
            if bool(latest.get("market_hours_ok"))
            else "UNAVAILABLE"
        ),
    )
    observed_option_data = _has_observed_option_data(latest)
    ...
    option_quote_status = _diagnostic_status(
        ...
        default=(
            ...
            if latest.get("option_quote_timestamp") not in (None, "")
            else "UNAVAILABLE"
        ),
    )
