...
class PaperMarketDataStaleError(PaperMarketDataUnavailableError): pass
...
class PaperMarketDataDiagnosticError(PaperMarketDataCorruptionError):
    def __init__(
        self,
        *,
        stage: MarketDataReadinessStage,
        reason_code: str,
        source_component: str = "paper_market_data",
        ...
        if type(stage) is not MarketDataReadinessStage:
            raise ValueError("stage must be MarketDataReadinessStage")
        self.stage = stage
        self.diagnostic = MarketDataReadinessDiagnostic(
            ...
            ce_count=ce_count,
            pe_count=pe_count,
            common_expiry_count=common_expiry_count,
            unique_strike_count=unique_strike_count,
        )
        super().__init__(f"{stage.value}:{reason_code}")
...
def verify_timestamp_freshness(*, timestamp: datetime, evaluated_at: datetime, maximum_age_seconds: float, future_tolerance_seconds: float = 2.0) -> None:
    ...
    if age > maximum_age: raise PaperMarketDataStaleError("provider evidence is stale")
