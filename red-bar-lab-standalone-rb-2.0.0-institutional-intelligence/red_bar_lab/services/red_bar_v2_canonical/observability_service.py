...
def _freshness(
    record: ObservabilityResolutionRecord,
    now: datetime,
) -> tuple[str, float | None]:
    if now.tzinfo is None or now.utcoffset() is None:
        raise ValueError("now must be timezone-aware")
    current = now.astimezone(INDIA_MARKET_TZ)
    event_time = _require_aware(
        record.envelope.section_2.evaluation_timestamp,
        "event timestamp",
    )
    ...
    if event_time.date() != trading_date:
        raise CanonicalPersistenceCorruptionError(
            "event timestamp market date mismatch"
        )
    if trading_date < current.date():
    ...
    if age < -CLOCK_SKEW_TOLERANCE_SECONDS:
        raise CanonicalPersistenceCorruptionError(
            "event timestamp materially in the future"
        )
    age = max(age, 0.0)
    threshold = float(
        record.envelope.section_1.timestamps.maximum_age_seconds
    )
...
def _section_1(record: ObservabilityResolutionRecord) -> CanonicalSection1View:
    ...
    if outcome == "REFERENCE_READY":
        explanation = (
            "The persisted reference, NIFTY evidence and futures VWAP context "
            "were available at event time."
        )
    elif outcome == "REFERENCE_WAITING":
        explanation = (
            "The persisted event could not evaluate direction because its "
            "reference was not ready."
        )
    elif outcome in {"CANDLES_STALE", "SESSION_MISALIGNED"}:
        explanation = (
            "Persisted evidence existed, but its event-time timestamps were "
            "stale or misaligned."
        )
    else:
...
def _section_2(record: ObservabilityResolutionRecord) -> CanonicalSection2View:
    ...
    if decision.rsi is not None:
        evidence.append(
            CanonicalEvidenceView(
                "RSI",
                f"{decision.rsi.value:.2f}",
                f"> {decision.rsi.bullish_threshold:.2f} bullish / "
                f"< {decision.rsi.bearish_threshold:.2f} bearish",
                _alignment(
                    decision.rsi.bullish_aligned,
                    decision.rsi.bearish_aligned,
                ),
            )
        )
    if decision.futures_vwap is not None:
        evidence.append(
            CanonicalEvidenceView(
                "Futures vs VWAP",
                f"Price {decision.futures_vwap.comparison_price:.2f} / "
                f"VWAP {decision.futures_vwap.vwap:.2f}",
                "Price above VWAP bullish / below VWAP bearish",
                _alignment(
                    decision.futures_vwap.bullish_aligned,
                    decision.futures_vwap.bearish_aligned,
                ),
            )
        )
    if decision.midpoint is not None:
        evidence.append(
            CanonicalEvidenceView(
                "Index vs midpoint",
                f"Close {decision.midpoint.index_close:.2f} / "
                f"Midpoint {decision.midpoint.midpoint:.2f}",
                "Close above midpoint bullish / below midpoint bearish",
                _alignment(
                    decision.midpoint.bullish_aligned,
                    decision.midpoint.bearish_aligned,
                ),
            )
        )
...
def _section_3(
    ...
    if bundle is None:
        return CanonicalSection3View(
            ...
            None,
            (),
            "No bundle was created because canonical admission was not ALLOWED.",
        )
    projected = tuple(
        CanonicalBundleEventView(
    ...
    return CanonicalSection3View(
        ...
        explanation=(
            "RED BAR V2 CANONICAL BUNDLE is available as immutable evidence. "
            "AVAILABLE does not mean executed."
        ),
    )
...
def _parity(record: ObservabilityResolutionRecord) -> CanonicalParityView:
    ...
    if parity is None:
        return CanonicalParityView(
            ...
            (),
            (),
            "No persisted parity comparison is available for this observation.",
        )
    values = (
        (
            "Direction",
            parity.legacy_direction,
            _value(parity.canonical_direction),
            "direction",
        ),
        (
            "Option side",
            parity.legacy_option_side,
            _value(parity.canonical_option_side),
            "option_side",
        ),
        (
            "Admission",
            str(parity.legacy_allowed),
            str(parity.canonical_allowed),
            "admission",
        ),
        (
            "Entry type",
            parity.legacy_entry_type,
            _value(parity.canonical_entry_type),
            "entry_type",
        ),
        (
            "Timeframe",
            parity.legacy_timeframe,
            parity.canonical_timeframe,
            "timeframe",
        ),
        (
            "Trend strength",
            parity.legacy_trend_strength,
            _value(parity.canonical_trend_strength),
            "trend_strength",
        ),
        (
            "Admission code",
            parity.legacy_admission_code,
            parity.canonical_admission_code,
            "admission_code",
        ),
    )
    rows = tuple(
        CanonicalParityRow(
            ...
            str(canonical) if canonical is not None else "—",
            "MISMATCH" if key in parity.mismatches else "MATCH",
        )
    ...
    return CanonicalParityView(
        ...
        (
            ...
            if parity.matches
            else "Mismatch is observational architecture evidence only; "
            "legacy execution remained unchanged."
        ),
    )


def _history(
    records: tuple[ObservabilityResolutionRecord, ...],
    now: datetime,
) -> tuple[CanonicalHistoryRow, ...]:
    ...
    for record in records:
        ...
        rows.append(
            CanonicalHistoryRow(
                event_time=envelope.section_2.evaluation_timestamp.astimezone(
                    INDIA_MARKET_TZ
                ).isoformat(),
                trading_date=envelope.trading_date.isoformat(),
                section_1_outcome=(
                    _value(envelope.section_1.outcome) or "UNAVAILABLE"
                ),
                admission_outcome=(
                    _value(envelope.section_2.admission_outcome)
                    or "UNAVAILABLE"
                ),
                ...
                parity=(
                    "NOT AVAILABLE"
                    if envelope.parity is None
                    else "MATCH" if envelope.parity.matches else "MISMATCH"
                ),
...
class RedBarV2CanonicalObservabilityService:
    """Build immutable read-only UI projections from persisted evidence."""

    def __init__(
        self,
        repository: RedBarV2CanonicalObservabilityRepository,
        *,
        database_path: Path,
    ) -> None:
    ...
    def load(
        ...
        *,
        ...
        short_path = (
            f"…/{self._database_path.parent.name}/{self._database_path.name}"
        )
        if current.tzinfo is None or current.utcoffset() is None:
            return _empty_status(
                availability="CANONICAL_READ_FAILED",
                feature_enabled=feature_enabled,
                freshness="UNAVAILABLE",
                reason_code="NAIVE_OBSERVABILITY_CLOCK",
                error_category="ValueError",
                database_display=short_path,
            )
        if not feature_enabled:
            return _empty_status(
                availability="SHADOW_DISABLED",
        ...
        try:
            ...
            if not records:
                return _empty_status(
                    availability="WAITING_FOR_FIRST_OBSERVATION",
                    feature_enabled=True,
                    freshness="UNAVAILABLE",
                    reason_code="NO_CANONICAL_OBSERVATIONS",
                    error_category=None,
                    database_display=short_path,
                )
            latest = records[0]
            expected_bundle = latest.envelope.section_3
            if expected_bundle is None:
                events: tuple[CanonicalBundleLifecycleEvent, ...] = ()
            else:
                selected = self._repository.selected_bundle_evidence(
            ...
            delay = (
                latest.persisted_at
                - latest.envelope.section_2.evaluation_timestamp
            ).total_seconds()
            persistence = CanonicalPersistenceView(
                ...
                explanation=(
                    "Digest, canonical schema, selected-bundle equality and "
                    "lifecycle validation succeeded during the read-only query."
                ),
            )
            status = CanonicalShadowPageStatus(
            ...
            return CanonicalShadowObservationView(
                ...
                _section_1(latest),
                _section_2(latest),
                _section_3(latest, events),
                _parity(latest),
                persistence,
                _history(records, current),
            )
        except CanonicalPersistenceCorruptionError as exc:
            return _empty_status(
                availability="CANONICAL_DATA_CORRUPT",
                feature_enabled=True,
                freshness="CORRUPT",
                reason_code="CANONICAL_DATA_CORRUPT",
                error_category=type(exc).__name__,
                database_display=short_path,
            )
        except CanonicalPersistenceUnavailableError as exc:
            return _empty_status(
                availability="CANONICAL_DATABASE_UNAVAILABLE",
                feature_enabled=True,
                freshness="UNAVAILABLE",
                reason_code="CANONICAL_DATABASE_UNAVAILABLE",
                error_category=type(exc).__name__,
                database_display=short_path,
            )
