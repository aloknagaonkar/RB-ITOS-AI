...
def _decode_reservation(row: sqlite3.Row) -> CanonicalBundleReservation:
    ...
    try:
        ...
        reservation = CanonicalBundleReservation(
            ...
            released_at=_aware_iso(data["released_at"], "released_at") if data.get("released_at") else None,
...
def _decode_event(row: sqlite3.Row) -> CanonicalReservationLifecycleEvent:
    ...
    try:
        ...
        event = CanonicalReservationLifecycleEvent(
            ...
            event_timestamp=_aware_iso(row["event_timestamp"], "reservation event_timestamp"),
    ...
    if expected_state is None or metadata.get("state") != expected_state:
        raise ReservationCorruptionError("reservation event metadata mismatch: state")
    return event
...
def _validate_chain(reservation: CanonicalBundleReservation, events: tuple[CanonicalReservationLifecycleEvent, ...]) -> None:
    ...
    for event in events:
        if event.reservation_id != reservation.reservation_id:
            raise ReservationCorruptionError("reservation event reservation mismatch")
        ...
        if event.event_timestamp < reservation.reserved_at:
            raise ReservationCorruptionError("reservation event precedes reservation")
    acquired = [event for event in events if event.event_type is ReservationEventType.RESERVATION_ACQUIRED]
    if len(acquired) != 1 or events[0].event_type is not ReservationEventType.RESERVATION_ACQUIRED:
    ...
    if terminal and events[-1] != terminal[0]:
        raise ReservationCorruptionError("reservation has events after terminal transition")
    if reservation.state is ReservationState.RESERVED:
        if terminal:
            raise ReservationCorruptionError("reserved state conflicts with terminal lifecycle event")
        return
    if reservation.state is ReservationState.REJECTED:
        raise ReservationCorruptionError("rejected reservation workflow is unsupported")
    if len(terminal) != 1 or reservation.released_at is None or reservation.release_reason is None:
        raise ReservationCorruptionError("terminal reservation lifecycle event missing")
    event = terminal[0]
    if reservation.state is ReservationState.RELEASED:
        if event.event_type is not ReservationEventType.RESERVATION_RELEASED:
            raise ReservationCorruptionError("released state lifecycle mismatch")
        if not _same_instant(event.event_timestamp, reservation.released_at):
            raise ReservationCorruptionError("released event timestamp mismatch")
        ...
        if event.event_timestamp >= reservation.lease_expires_at:
            raise ReservationCorruptionError("released event occurred after lease expiry")
    elif reservation.state is ReservationState.EXPIRED:
        if event.event_type is not ReservationEventType.RESERVATION_EXPIRED:
            raise ReservationCorruptionError("expired state lifecycle mismatch")
        if not _same_instant(event.event_timestamp, reservation.lease_expires_at):
            raise ReservationCorruptionError("expired event timestamp mismatch")
        if event.reason_code != "LEASE_EXPIRED" or reservation.release_reason != "LEASE_EXPIRED":
            raise ReservationCorruptionError("expired event reason mismatch")
        if not _same_instant(reservation.released_at, reservation.lease_expires_at):
...
def verify_reservation_evidence(conn: sqlite3.Connection, *, reservation_id: str, expected_bundle_id: str | None = None) -> VerifiedReservationEvidence:
    ...
    return VerifiedReservationEvidence(reservation=reservation, events=events)
