...
class ReservationStorageError(Exception):
    """Reservation storage could not complete safely."""


class SQLiteCanonicalReservationRepository:
    """Atomic ownership-lease repository; no execution or portfolio authority."""
    ...
    def _verify_bundle(
        ...
        bundle_id: str,
    ) -> tuple[RedBarV2SignalBundle, bool]:
        row = conn.execute(
        ...
        for event_row in rows:
            ...
            if event.bundle_id != bundle_id:
                raise CanonicalPersistenceCorruptionError(
                    "lifecycle event bundle identity mismatch"
                )
            has_available = has_available or event.event_type.value == "BUNDLE_AVAILABLE"
        return bundle, has_available
    ...
    def _from_row(row: sqlite3.Row) -> CanonicalBundleReservation:
        ...
        if reservation_sha256(payload) != str(row["payload_sha256"]):
            raise CanonicalPersistenceCorruptionError(
                "reservation payload digest mismatch"
            )
        try:
        ...
        try:
            ...
            try:
                ...
                eligibility = evaluate_reservation_eligibility(
                    ...
                    has_bundle_available_event=has_available,
                )
    ...
    def _event(
        ...
        metadata = canonical_reservation_json(
            {
                "reservation_id": reservation.reservation_id,
                "bundle_id": reservation.bundle_id,
                "state": reservation.state.value,
        ...
        conn.execute(
            """
            INSERT OR IGNORE INTO canonical_red_bar_v2_bundle_reservation_events(
