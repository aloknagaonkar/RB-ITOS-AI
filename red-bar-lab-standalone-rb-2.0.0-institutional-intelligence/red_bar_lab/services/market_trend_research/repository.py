...
from .models import DualPcrResearchSnapshot, OptionOiCell, PcrWindowDefinition
...
class MarketTrendResearchRepository:
    ...
    def persist(self, snapshot: DualPcrResearchSnapshot) -> None:
        payload = json.dumps(asdict(snapshot), default=_json_value, sort_keys=True, separators=(",", ":"))
        ...
        with sqlite3.connect(self.path) as connection:
            ...
            connection.execute(
                """INSERT OR REPLACE INTO market_trend_research_snapshots
                   (snapshot_id, underlying, trading_date, source_timestamp, evaluated_at, state, payload_json)
                   VALUES (?,?,?,?,?,?,?)""",
                (snapshot.snapshot_id, snapshot.underlying, snapshot.trading_date.isoformat(), snapshot.source_timestamp.isoformat(), snapshot.evaluated_at.isoformat(), snapshot.quality.state.value, payload),
            )
            connection.execute(
                """DELETE FROM market_trend_research_snapshots WHERE snapshot_id IN (
                   SELECT snapshot_id FROM market_trend_research_snapshots
                   WHERE underlying=? ORDER BY evaluated_at DESC LIMIT -1 OFFSET ?)""",
    ...
    def create_anchor(
        ...
        *,
        ...
        window_json = json.dumps(asdict(window), default=_json_value, sort_keys=True)
        cells_json = json.dumps([asdict(cell) for cell in cells], default=_json_value, sort_keys=True)
        self.path.parent.mkdir(parents=True, exist_ok=True)
        with sqlite3.connect(self.path) as connection:
            connection.executescript(_SCHEMA)
            cursor = connection.execute(
                """INSERT OR IGNORE INTO market_trend_research_anchors
                   (underlying, trading_date, anchor_timestamp, spot, window_json, cells_json)
                   VALUES (?,?,?,?,?,?)""",
                (underlying, trading_date.isoformat(), anchor_timestamp.isoformat(), spot, window_json, cells_json),
            )
            connection.commit()
            return cursor.rowcount == 1

    def load_anchor(self, *, underlying: str, trading_date: date) -> dict[str, Any] | None:
        if not self.path.exists(): return None
        try:
            with sqlite3.connect(self.path) as connection:
                row = connection.execute(
                    """SELECT anchor_timestamp, spot, window_json, cells_json
                       FROM market_trend_research_anchors WHERE underlying=? AND trading_date=?""",
        ...
        return {"anchor_timestamp": row[0], "spot": row[1], "window": json.loads(row[2]), "cells": json.loads(row[3])}
