...
def test_completed_boundary_starts_at_first_regular_session_close() -> None:
    assert completed_five_minute_close(datetime(2026, 8, 25, 9, 19, 59, tzinfo=IST)) is None
    assert completed_five_minute_close(
        datetime(2026, 8, 25, 9, 22, 17, tzinfo=IST)
    ) == datetime(2026, 8, 25, 9, 20, tzinfo=IST)


def test_evidence_older_than_candle_close_is_not_attached() -> None:
    observation = build_five_minute_pcr_observation(
        projection=_projection(datetime(2026, 8, 25, 9, 19, 59, tzinfo=IST)),
        combined=_combined(),
        evaluated_at=datetime(2026, 8, 25, 9, 20, 2, tzinfo=IST),
    )
...
def test_repository_keeps_one_immutable_row_per_candle(tmp_path) -> None:
    observation = build_five_minute_pcr_observation(
        projection=_projection(datetime(2026, 8, 25, 9, 20, 1, tzinfo=IST)),
        combined=_combined(),
        evaluated_at=datetime(2026, 8, 25, 9, 20, 2, tzinfo=IST),
        rsi=54.25,
        vwap=24211.75,
    ...
    assert repository.persist_five_minute_pcr_observation(observation) is True
    assert repository.persist_five_minute_pcr_observation(observation) is False
    rows = repository.five_minute_pcr_history(
        underlying="NIFTY 50",
    ...
    assert rows[0]["candle_close_timestamp"].startswith("2026-08-25T09:20:00")


def test_rsi_uses_only_candles_completed_by_boundary() -> None:
    timestamps = pd.date_range("2026-08-25 09:15", periods=17, freq="5min", tz=IST)
    candles = pd.DataFrame({
        "timestamp": timestamps,
        "close": [100.0 + value for value in range(17)],
    })

    value = completed_five_minute_rsi(
        candles,
        candle_close=datetime(2026, 8, 25, 10, 30, tzinfo=IST),
    )

    assert value == 100.0


def test_futures_vwap_must_be_aligned_to_boundary() -> None:
    close = datetime(2026, 8, 25, 10, 30, tzinfo=IST)
    rows = [{
        "futures_vwap": 24210.5,
        "futures_vwap_timestamp": close.isoformat(),
    }]

    assert aligned_futures_vwap(rows, candle_close=close) == 24210.5
