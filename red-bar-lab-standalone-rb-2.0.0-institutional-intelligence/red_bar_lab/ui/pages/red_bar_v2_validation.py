...
def _render_shadow_observability(settings, instrument_key: str) -> None:
    ...
    st.warning(
        "CANONICAL SHADOW MODE — Read-only observational architecture. "
        "Legacy Red Bar V2 owns trade execution. This page cannot create, block, "
        "modify or exit a trade."
    )
    st.error(
        "NO — canonical processing is observational only. "
        "Legacy Red Bar V2 remains execution authority."
    )
    ...
    if status.latest_event_timestamp is not None:
        st.caption(
            ...
            f"Age: {_text(round(status.age_seconds, 2) if status.age_seconds is not None else None)} seconds"
        )

    if status.availability == "CANONICAL_DATA_CORRUPT":
        st.error("Persisted canonical evidence failed digest/schema validation. Untrusted evidence is not rendered or repaired.")
        return
    if status.availability in {"SHADOW_DISABLED", "WAITING_FOR_FIRST_OBSERVATION", "CANONICAL_DATABASE_UNAVAILABLE"}:
        st.info(
            "No trusted canonical observation is available yet. Opening or refreshing "
            "this page does not initialize schema or start shadow processing."
        )
        return

    section_1 = view.section_1
    section_2 = view.section_2
    section_3 = view.section_3
    parity = view.parity
    ...
    readiness = pd.DataFrame(
        ...
        columns=["Field", "Persisted event-time value"],
        dtype="string",
    )
    ...
    st.caption(
        f"State: {section_2.previous_state} → {section_2.current_state} | "
        f"Timeframe: {section_2.evaluation_timeframe} | "
        f"Trend strength: {_text(section_2.trend_strength)}"
    )
    if section_2.evidence:
        evidence = pd.DataFrame(
            [asdict(item) for item in section_2.evidence],
            columns=["name", "numeric_value", "required_interpretation", "actual_alignment"],
        ).astype("string")
    ...
    if section_3.bundle_available:
        bundle = pd.DataFrame(
            [
                ...
                ("Lifecycle status", _text(section_3.lifecycle_status)),
            ],
            columns=["Field", "Value"],
            dtype="string",
        )
    ...
    history = pd.DataFrame([asdict(row) for row in view.history])
    if not history.empty:
        history = history.astype("string")
    ...
    with st.expander("7. What happened from signal detection to shadow persistence?"):
        ...
        st.caption(
            f"Selected observation: {persistence.resolution_id}; admission "
            f"{section_2.admission_outcome}; parity {parity.overall}."
        )
