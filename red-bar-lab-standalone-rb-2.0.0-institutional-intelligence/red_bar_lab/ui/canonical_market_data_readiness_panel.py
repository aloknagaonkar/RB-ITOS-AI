...
def render_canonical_market_data_readiness_panel(st, settings) -> None:
    ...
    if observation.report is None:
        messages = {
            ...
            "READINESS_REPORT_CORRUPT": "The readiness report failed schema or digest validation.",
    ...
    fields = [
        ...
        ("Evaluated timestamp", _text(report.evaluated_at)), ("NIFTY spot", _text(report.spot_price)),
        ("Spot provider timestamp", _text(report.spot_timestamp)), ("Expiry", _text(report.expiry)),
        ("Detected strike interval", _text(report.strike_interval)), ("ATM strike", _text(report.atm_strike)),
