...
def _alignment_result(
    option_time: datetime | None,
    futures_market_time: datetime | None,
    underlying_time: datetime | None,
) -> dict[str, Any]:
    times = [option_time, futures_market_time, underlying_time]
    if any(value is None for value in times):
        return {
            "source": "Cross-source alignment",
            "timestamp": None,
            "age_seconds": None,
            "warning_seconds": None,
            "limit_seconds": _MAX_COMPLETED_5M_ALIGNMENT_GAP_SECONDS,
            "status": "MISSING",
            "reason_code": None,
        }
    ...
    return {
        ...
        "warning_seconds": None,
        "limit_seconds": _MAX_COMPLETED_5M_ALIGNMENT_GAP_SECONDS,
        "status": "MISALIGNED" if gap > _MAX_COMPLETED_5M_ALIGNMENT_GAP_SECONDS else "PASS",
        "reason_code": "MARKET_TIMESTAMPS_MISALIGNED" if gap > _MAX_COMPLETED_5M_ALIGNMENT_GAP_SECONDS else None,
    }
...
def build_market_at_a_glance(
    ...
    *,
    ...
    freshness_diagnostics = [
        _freshness_result(
            "Option snapshot", option_time, now=current_time,
            warning_seconds=_OPTION_WARNING_SECONDS,
            stale_seconds=_OPTION_MAX_AGE_SECONDS,
            missing_code="OPTION_TIMESTAMP_MISSING",
            stale_code="OPTION_SNAPSHOT_STALE",
        ),
        _freshness_result(
            "Futures collection", futures_collection_time, now=current_time,
            warning_seconds=_FUTURES_COLLECTION_WARNING_SECONDS,
            stale_seconds=_FUTURES_COLLECTION_MAX_AGE_SECONDS,
            missing_code="FUTURES_COLLECTION_TIMESTAMP_MISSING",
            stale_code="FUTURES_COLLECTOR_STALE",
        ),
        _freshness_result(
            "Futures candle", futures_market_time, now=current_time,
            warning_seconds=None,
            stale_seconds=_COMPLETED_5M_MAX_AGE_SECONDS,
            missing_code="FUTURES_MARKET_CANDLE_MISSING",
            stale_code="FUTURES_MARKET_CANDLE_STALE",
        ),
        _freshness_result(
            "Underlying candle", underlying_time, now=current_time,
            warning_seconds=None,
            stale_seconds=_COMPLETED_5M_MAX_AGE_SECONDS,
            missing_code="UNDERLYING_CANDLE_MISSING",
            stale_code="UNDERLYING_CANDLE_STALE",
        ),
    ...
    spot_vwap_conflict = (
        spot_vwap_direction in {"BULLISH", "BEARISH"}
        and observed_direction in {"BULLISH", "BEARISH"}
        and spot_vwap_direction != observed_direction
    )
    futures_vwap_conflict = (
        futures_vwap_direction in {"BULLISH", "BEARISH"}
        and observed_direction in {"BULLISH", "BEARISH"}
        and futures_vwap_direction != observed_direction
    )
    strong_futures_conflict = (
        futures_direction in {"BULLISH", "BEARISH"}
        and observed_direction in {"BULLISH", "BEARISH"}
        and futures_direction != observed_direction
        and futures_strength in _CONFIRMING_STRENGTH
    )
    weak_futures_opposition = (
        futures_direction in {"BULLISH", "BEARISH"}
        and observed_direction in {"BULLISH", "BEARISH"}
        and futures_direction != observed_direction
        and futures_strength not in _CONFIRMING_STRENGTH
    )
    ...
    option_conflict = (
        option_direction in {"BULLISH", "BEARISH"}
        and observed_direction in {"BULLISH", "BEARISH"}
        and option_direction != observed_direction
    )
    rsi_conflict = (
        rsi_view in {"BULLISH", "BEARISH"}
        and observed_direction in {"BULLISH", "BEARISH"}
        and rsi_view != observed_direction
    )
    ...
    trade_eligibility = (
        ...
        and contract_quality == "PASS"
        and direction_state == "CONFIRMED"
        and observed_direction in {"BULLISH", "BEARISH"}
        else "BLOCKED"
    )
    trade_bias = (
        "BUY CE" if trade_eligibility == "ELIGIBLE" and observed_direction == "BULLISH"
        else "BUY PE" if trade_eligibility == "ELIGIBLE" and observed_direction == "BEARISH"
        else "WAIT"
    )

    if direction_state == "CONFIRMED":
        market_state = f"CONFIRMED {observed_direction}"
    elif direction_state == "CONFIRMED_WITH_CAUTION":
        market_state = f"CONFIRMED {observed_direction} WITH CAUTION"
    elif direction_state == "EARLY":
        market_state = f"EARLY {observed_direction} TRANSITION"
    elif direction_state == "CONFLICTED":
        market_state = "CONFLICTED"
    elif direction_state == "NEUTRAL":
        market_state = underlying_state if underlying_state.startswith("SIDEWAYS") else "SIDEWAYS"
    else:
        market_state = "UNAVAILABLE"

    confirmation = (
        f"Blocked by {primary_blocker}." if primary_blocker
        else "Direction is conflicted." if direction_state == "CONFLICTED"
        else "Direction is confirmed with caution." if direction_state == "CONFIRMED_WITH_CAUTION"
        else "Direction is structurally confirmed and evidence is ready." if trade_eligibility == "ELIGIBLE"
        else "Direction is visible but not trade-eligible."
    )

    latest_complete_evidence_time = max(
        [value for value in (futures_market_time, underlying_time) if value is not None],
        default=None,
    )
    freshness_rows = [
        {
            ...
            "Warning": "—" if item["warning_seconds"] is None else f"{item['warning_seconds']:.0f}s",
            "Limit": f"{item['limit_seconds']:.0f}s",
            "Result": item["status"],
        }
        for item in freshness_diagnostics
    ]
    ...
    checklist = [
        ...
        {"Check": "Spot VWAP acceptance", "Live value": spot_vwap_state, "Rule": "Available VWAP must not contradict structure", "Status": spot_vwap.get("reason") or "—"},
        {"Check": "RSI slope", "Live value": f"{_score(underlying.get('rsi'))} / {_score(underlying.get('rsi_slope'))}", "Rule": "Directional confirmation or conflict", "Status": rsi_view},
        {"Check": "Option pressure", "Live value": f"CE {_score(bullish)} / PE {_score(bearish)} / gap {_score(gap)}", "Rule": "Normalized ATM-distance pressure", "Status": option_direction},
        ...
        {"Check": "Futures VWAP", "Live value": futures_vwap_state, "Rule": "Used when available", "Status": futures_vwap.get("reason") or "—"},
    ]

    return {
        "as_of_timestamp": current_time.isoformat(),
        ...
        "underlying_direction": observed_direction,
        "acceptance_state": structural_state,
        ...
        "evidence_status": evidence_readiness,
        ...
        "latest_complete_evidence_time": latest_complete_evidence_time.isoformat() if latest_complete_evidence_time else None,
        "checklist": checklist,
        "explanation": (
            f"Observed {observed_direction}; structure {structural_state}; direction {direction_state}; "
            f"evidence {evidence_readiness}; contract quality {contract_quality}; trade {trade_eligibility}."
        ),
    }
...
def render_market_at_a_glance(settings, underlying_name: str) -> None:
    ...
    st.caption(
        "Observed direction is separated from freshness and trade eligibility. "
        "Stale evidence blocks trading without hiding completed market structure."
    )
    ...
    st.caption(
        "Observational only. Scores and thresholds are normalized heuristics pending historical outcome calibration. "
        "This panel does not modify Red Bar execution."
    )
    st.markdown("---")
