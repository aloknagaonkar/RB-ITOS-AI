        ...
        output.append(
            {
                ...
                "CE since-open ΔOI": _signed(
                    _field(row, "ce_opening_change")
                ),
                "CE since-open ΔOI%": _percent(
                    _field(row, "ce_opening_change_pct")
                ),
                "PE current OI": _indian(_field(row, "pe_current_oi")),
                "PE opening OI": _indian(_field(row, "pe_opening_oi")),
                "PE since-open ΔOI": _signed(
                    _field(row, "pe_opening_change")
                ),
                "PE since-open ΔOI%": _percent(
                    _field(row, "pe_opening_change_pct")
                ),
            }
        )
    return output
        ...
        output.append(
            {
                ...
                "Position": _position(row.get("position")),
                "CE current OI": _indian(_field(row, "ce_current_oi")),
                "CE previous-day OI": _indian(
                    _field(row, "ce_previous_day_oi")
                ),
                "CE day ΔOI": _signed(
                    _field(row, "ce_previous_day_change")
                ),
                "CE day ΔOI%": _percent(
                    _field(row, "ce_previous_day_change_pct")
                ),
                "PE current OI": _indian(_field(row, "pe_current_oi")),
                "PE previous-day OI": _indian(
                    _field(row, "pe_previous_day_oi")
                ),
                "PE day ΔOI": _signed(
                    _field(row, "pe_previous_day_change")
                ),
                "PE day ΔOI%": _percent(
                    _field(row, "pe_previous_day_change_pct")
                ),
            }
        )
    return output
        ...
        output.append(
            {
                "Strike": str(row.get("strike", "Not available")),
                "Position": _position(row.get("position")),
                "CE previous refresh OI": _indian(
                    _field(row, "ce_previous_refresh_oi")
                ),
                "CE refresh ΔOI": _signed(
                    _field(row, "ce_previous_refresh_change")
                ),
                "CE refresh ΔOI%": _percent(
                    _field(row, "ce_previous_refresh_change_pct")
                ),
                "PE previous refresh OI": _indian(
                    _field(row, "pe_previous_refresh_oi")
                ),
                "PE refresh ΔOI": _signed(
                    _field(row, "pe_previous_refresh_change")
                ),
                "PE refresh ΔOI%": _percent(
                    _field(row, "pe_previous_refresh_change_pct")
                ),
            }
        )
    return output
...
def _range(panel: dict[str, Any]) -> str:
    ...
    return (
        "Not available"
        if not strikes
        else f"{min(strikes):.0f}–{max(strikes):.0f}"
    )
...
def _render_morning(
    ...
    *,
    ...
    if not panel:
        ...
        st.info(message)
        st.write(
            f"Reference-level status: {reference.get('status') or 'Not available'}"
        )
        st.write(
            f"OI-baseline status: {baseline.get('status') or 'Not available'}"
        )
        return
    ...
    summary = [
        {
            "Field": "Reference-level status",
            "Value": reference.get("status") or "Not available",
        },
        {
            "Field": "Reference NIFTY level",
            "Value": _number(reference.get("reference_spot"), 2),
        },
        {
            "Field": "Reference timestamp",
            "Value": reference.get("reference_timestamp") or "Not available",
        },
        {
            "Field": "Fixed ATM",
            "Value": _number(reference.get("fixed_atm"), 0),
        },
        {
            "Field": "Actual expiry",
            "Value": panel.get("expiry") or "Not available",
        },
        {
            "Field": "Sessions to expiry",
            "Value": str(panel.get("sessions_to_expiry", "Not available")),
        },
        {
            "Field": "Window",
            "Value": f"ATM ±{panel.get('window_steps', 'Not available')}",
        },
        {"Field": "Fixed strike range", "Value": _range(panel)},
        {
            ...
            "Value": (
                ...
                f"{panel.get('observed_contract_count', 0)}"
            ),
        },
        {
            "Field": "Opening OI baseline timestamp",
            "Value": baseline.get("baseline_timestamp") or "Not available",
        },
        {
            "Field": "Current snapshot timestamp",
            "Value": panel.get("source_timestamp") or "Not available",
        },
        {
            "Field": "Source age",
            "Value": f"{_number(quality.get('source_age_seconds'), 1)} seconds",
        },
        {"Field": "PCR", "Value": _number(aggregate.get("pcr"))},
        {
            "Field": "PCR directional evidence",
            "Value": _bias(aggregate.get("classification"), stale=stale),
        },
    ]
    st.dataframe(_arrow_safe_rows(summary), width="stretch", hide_index=True)
    st.dataframe(
        _arrow_safe_rows(_morning_rows(panel)),
        width="stretch",
        hide_index=True,
    )
    total = _total(panel)
...
def _render_refresh_diagnostics(panel: dict[str, Any]) -> None:
    ...
    with st.expander(
        "Short-term OI movement since previous refresh",
        expanded=False,
    ):
        st.caption(
        ...
        if current_timestamp and previous_timestamp:
            try:
                ...
                elapsed = str(
                    (
                        datetime.fromisoformat(str(current_timestamp))
                        - datetime.fromisoformat(str(previous_timestamp))
                    ).total_seconds()
                )
        ...
        st.dataframe(
            _arrow_safe_rows(details),
            width="stretch",
            hide_index=True,
        )
        st.dataframe(
            _arrow_safe_rows(_refresh_rows(panel)),
            width="stretch",
            hide_index=True,
        )
...
def _render_current(
    ...
    *,
    ...
    summary = [
        {
            "Field": "Current NIFTY level",
            "Value": _number(panel.get("spot"), 2),
        },
    ...
    st.dataframe(_arrow_safe_rows(summary), width="stretch", hide_index=True)
    st.dataframe(
        _arrow_safe_rows(_current_rows(panel)),
        width="stretch",
        hide_index=True,
    )
    total = _total(panel)
...
def render_market_trend_research_panel(
    ...
    *,
    ...
    quality = projection.get("quality") or {}
    source_age = quality.get("source_age_seconds")
    stale = quality.get("state") == "STALE"
    if stale:
        st.warning(
            "STALE DATA — last research snapshot is "
            f"{_number(source_age, 1)} seconds old. "
    ...
    _render_morning(projection, stale=stale)
    _render_current(projection, stale=stale)
