...
export default function App() {
  ...
  return <div className="shell">
    <aside><div className="brand"><span className="brand-icon">M</span><div>MARKET LAB<small>STRATEGY RESEARCH</small></div></div>
      <div className="nav-label">WORKSPACE</div><nav>{['PCR workspace','Live shadow','Historical research','Data health','Configuration'].map(label =>
        <button key={label} className={tab===label ? 'active':''} onClick={()=>setTab(label)}><span>{label==='PCR workspace'?'◈':label==='Live shadow'?'◎':label==='Historical research'?'◫':label==='Data health'?'◉':'⚙'}</span>{label}</button>)}</nav>
    ...
    <main>
      ...
      <div className="page-heading"><div><div className="eyebrow">OPTIONS INTELLIGENCE</div><h1>{tab}</h1><p>{tab==='PCR workspace'?'One option chain. Two ATM perspectives. Every calculation traceable.':tab==='Live shadow'?'Observation-only strategy lifecycle, data health, entries, exits and P&L.':tab==='Historical research'?'Reconstruct Fixed, Moving, and Full PCR from expired option candles.':tab==='Data health'?'Connection health and data quality, with their limits visible.':'Versioned parameters for repeatable research.'}</p></div>
        {tab!=='Historical research' && tab!=='Live shadow' && <button disabled={busy} className={state.enabled?'secondary':'primary'} onClick={()=>void action(async()=>{
