...
export default function App() {
  ...
  useEffect(() => {
    let active = true
    const poll = async () => {
      try {const data = await api<State>('/state'); if (active) {setState(data); setDraft(old=>old ?? data.config); setError('')}}
    ...
    <main>
      ...
      {tab === 'PCR workspace' && <>
        ...
        <div className="metrics">{(['fixed','moving','full'] as const).map(mode=>{
          const result = latest?.evaluation.results.find(r=>r.mode===mode)
          return <article className="metric" key={mode} style={{borderTopColor:colors[mode]}}>
            ...
            <div className="metric-value">{result?.pcr == null ? '—' : result.pcr.toFixed(3)}<small>PCR</small></div>
