...
class BuySellStrengthEngine:
    ...
    def _velocity_alignment(row, velocity) -> float:
        if velocity is None:
            return 1.0
        v = velocity.change_5m_pct
        if v is None:
            return 1.0
        behaviour = getattr(row, "behaviour", "")
        aligned = ("BUILDUP" in behaviour and v > 0) or (("COVERING" in behaviour or "UNWINDING" in behaviour) and v < 0)
        return 1.15 if aligned else 0.90

    @classmethod
    def evaluate(cls, flow_rows: Iterable, velocity_by_key: dict[tuple[float, str], object] | None = None) -> BuySellStrength:
        ...
        for row in flow_rows:
            ...
            if bias == "BULLISH":
                bullish += score; directional_rows += 1
            elif bias == "BEARISH":
                bearish += score; directional_rows += 1
            else:
                neutral += score
        total = bullish + bearish + neutral
        if total <= 0:
            return BuySellStrength(0.0, 0.0, 100.0, 0.0, "LOW", 0.0, "No directional institutional evidence available.")
        ...
        return BuySellStrength(
            ...
            round(breadth, 2), f"Institutional {leader} evidence leads by {abs(net):.1f} percentage points across {directional_rows}/{total_rows} directional samples.",
        )
