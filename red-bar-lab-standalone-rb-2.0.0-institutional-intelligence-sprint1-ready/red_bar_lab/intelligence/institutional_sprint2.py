...
from red_bar_lab.intelligence.institutional_confidence import InstitutionalConfidenceEngine, InstitutionalConfidence
...
class InstitutionalSprint2Service:
    ...
    Every input is a persisted ONLINE option-chain snapshot. No candidate, order,
    committee, portfolio or execution state is mutated by this service.
    """
    ...
    def _snapshots(self, instrument_key: str, trading_date: str) -> list[tuple[pd.Timestamp, dict[str, object], pd.DataFrame]]:
    ...
    def latest(self, instrument_key: str, trading_date: str) -> InstitutionalSprint2Snapshot:
        ...
        if len(snapshots) < 2:
            empty_flow = InstitutionalOptionFlowEngine.evaluate_frames(pd.DataFrame(), pd.DataFrame())
            ...
            return InstitutionalSprint2Snapshot(None, None, None, empty_flow, (), (), rotation, strength, confidence,
                len(snapshots), "WAITING", "At least two ONLINE option-chain snapshots are required; 5-15 minutes of history improves Sprint-2 velocity confidence.")
        ...
        confidence = InstitutionalConfidenceEngine.evaluate(strength, flow.rows, velocity, premium, rotation)
        return InstitutionalSprint2Snapshot(
            current_ts.isoformat(), previous_ts.isoformat(), flow.option_expiry,
            flow, velocity, premium, rotation, strength, confidence, len(snapshots),
